...
void updateUserMenu(){
    ...
    printf("\nEnter user ID to change details >>");
    scanf("%d", &searchID);
...
void addBookMenu(){
    ...
    if(findBookInList(tempISBN) != NULL){
        ...
        printf("Error, ISBN cannot be 0, try again\n");
        return;
    }
...
void updateBookMenu(){
    ...
    printf("\nEnter book ISBN to change details >>");
    scanf("%d", &searchID);
    
    if((booktoupdate = findBookInList(searchID)) == NULL){
        printf("Book with ISBN %d not found, try again\n", searchID);
        return;
    }
...
void borrowBookMenu(){
   ...
   if(borrower == NULL){
    ...
    if(booktoborrow == NULL){
        ...
        printf("Book with ISBN %d is already borrowed, return it before borrowing again\n", tempISBN);
        return;
    }
    ...
    printf("Book %d borrowed successfully!\n", borrower->borrowedbooks[borrower->borrowcount - 1]); // Debugging step, struct access not necessary
...
void returnBookMenu(){
   ...
   if(borrower == NULL){
        ...
        printf("User %d has no books borrowed!\n", tempID);
        return;
    }
    ...
    if(booktoreturn == NULL){
        ...
        printf("Book with ISBN %d is not borrowed, borrow it before returning\n", tempISBN);
        return;
    }
